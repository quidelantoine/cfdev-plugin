...
### `hidden`
Champ caché (`<input type="hidden">`). La valeur est sauvegardée normalement via le cycle save habituel — elle passe par la validation standard. Ne pas marquer `required` sans `default_value`, sinon la validation échouera silencieusement.

**Cas d'usage typiques :**

```php
// Marquer l'origine d'un enregistrement (import, API, manuel…)
['id' => 'source', 'type' => 'hidden', 'default_value' => 'import']

// Verrouiller une valeur calculée à la création (ne jamais l'écraser en admin)
['id' => 'schema_version', 'type' => 'hidden', 'default_value' => '2']

// Stocker un identifiant externe sans l'exposer dans le formulaire
// La valeur est pré-remplie programmatiquement via un hook save_post amont
['id' => 'stripe_product_id', 'type' => 'hidden']

// Dans un Bundle : stocker un flag interne par ligne sans colonne visible
...
### `user_select`
Liste déroulante d'utilisateurs. Stocke l'ID de l'utilisateur. Accepte tous les args de `wp_dropdown_users()`, qui sont transmis à `WP_User_Query` — y compris `role__in` pour filtrer sur plusieurs rôles.
...
// Plusieurs rôles (role__in, WP_User_Query)
['id' => 'assignee', 'type' => 'user_select', 'label' => 'Assigné à', 'args' => [
    'role__in' => ['editor', 'author'], 'show_option_none' => '— Choisir —',
]]
```
